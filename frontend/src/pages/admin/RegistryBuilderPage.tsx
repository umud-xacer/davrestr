...
export function RegistryBuilderPage() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      setError(err.response?.data?.detail || t("registryBuilder.submitError"));
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-8">
      <h1 className="text-xl font-semibold text-slate-800">{t("registryBuilder.title")}</h1>
      <p className="mt-1 text-sm text-slate-500">{t("registryBuilder.desc")}</p>

      <form onSubmit={handleSubmit} className="mt-6 space-y-4 rounded-lg border border-slate-200 bg-white p-6">
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium text-slate-700">{t("registryBuilder.codeLabel")}</label>
            <input
              value={code}
              onChange={(e) => setCode(e.target.value.toUpperCase())}
              className="mt-1 w-full rounded-md border border-slate-300 px-3 py-2"
              placeholder={t("registryBuilder.codePlaceholder")}
              required
            />
          </div>
          <div>
            <label className="block text-sm font-medium text-slate-700">{t("registryBuilder.nameLabel")}</label>
            <input
              value={name}
        ...
        <div>
          <label className="block text-sm font-medium text-slate-700">{t("registryBuilder.descLabel")}</label>
          <textarea
            value={description}
        ...
        <div className="border-t border-slate-100 pt-4">
          <div className="flex items-center justify-between">
            <h3 className="font-medium text-slate-700">{t("registryBuilder.fieldsTitle")}</h3>
            <button
              type="button"
              onClick={() => setFields((prev) => [...prev, emptyField()])}
              className="rounded bg-slate-100 px-3 py-1 text-xs hover:bg-slate-200"
            >
              {t("registryBuilder.addField")}
            </button>
          </div>
                  ...
                  <div className="mt-2 space-y-2 border-t border-slate-100 pl-4 pt-2">
                    ...
                    {(f.item_fields || []).map((sf, sidx) => (
                      <div key={sidx} className="grid grid-cols-2 items-center gap-2 sm:grid-cols-12">
                        ...
                        />
                        <input
                          placeholder={t("registryBuilder.itemLabelPlaceholder")}
                          value={sf.label}
                        ...
                        <label className="col-span-1 flex items-center gap-1 text-xs">
                          ...
                          />
                          {t("registryBuilder.itemRequired")}
                        </label>
                        <button
                          type="button"
                    ...
                    >
                      {t("registryBuilder.addItemField")}
                    </button>
                  </div>
                )}
              </div>
          ...
          <p className="mt-2 text-xs text-slate-400">{t("registryBuilder.publicHint")}</p>
        </div>
        ...
        >
          {saving ? t("registryBuilder.saving") : t("registryBuilder.submit")}
        </button>
      </form>

      <div className="mt-8">
        <h2 className="font-medium text-slate-700">{t("registryBuilder.existingTitle")}</h2>
        <ul className="mt-3 divide-y divide-slate-200 rounded-lg border border-slate-200 bg-white">
          {types.map((rt) => (
            <li key={rt.id} className="p-3">
              <div className="flex items-center justify-between">
                <span className="font-medium">{rt.name}</span>
                <span className="text-xs text-slate-400">{rt.code}</span>
              </div>
              <p className="mt-1 text-xs text-slate-500">
                {rt.field_schema.length} {t("registryBuilder.fieldsCountSuffix")}
              </p>
            </li>
          ))}
        </ul>
