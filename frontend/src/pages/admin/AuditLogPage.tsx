...
export function AuditLogPage() {
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-8">
      <h1 className="text-xl font-semibold text-slate-800">Audit jurnali</h1>
      <p className="mt-1 text-sm text-slate-500">O'zgarmas (append-only) tizim harakatlari tarixi</p>
      ...
      <div className="mt-6 overflow-x-auto rounded-lg border border-slate-200 bg-white">
        <table className="w-full min-w-[640px] text-sm">
          ...
          <tbody className="divide-y divide-slate-100">
            {logs.map((l) => (
              <tr key={l.id}>
                <td className="p-3 text-xs text-slate-500">{new Date(l.created_at).toLocaleString("uz-UZ")}</td>
